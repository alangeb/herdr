---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Tau, TauRLM, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
herdr integration install tau
herdr integration install taurlm
...
herdr integration uninstall tau
herdr integration uninstall taurlm
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode, Tau, TauRLM | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Tau and TauRLM integrations are experimental and CLI-only. Install the lifecycle wrappers:
...
Launch the wrapper instead of the ordinary agent command, for example `tau-herdr` or `taurlm-herdr`. The wrapper detects the agent project from `TAU_ROOT`, `TAURLM_ROOT`, `~/<agent>`, or `../<agent>`, waits for its A2A lifecycle socket, and reports lifecycle state plus session identity through Herdr's socket API.
